...
先处理 "/"，处理函数为canonicalizedSnippetForJavaScript；然后看处理后的结果是否在 URL 中(isContainedInRequest)并且是否 URL 中存在 script 标签(m_scriptTagFoundInRequest)；如果不在，则设置当前状态为PermittingAdjacentCharacterTokens，即认为它是没有危险的。
